...
void main() {
	...
	printf("\n ptrArray[0]의 가리키는 (*ptrArray[0]) = %c", *ptrArray[0]);
	printf("\n ptrArray[0]의 가리키는문자열 (*ptrArray[0]) = %s \n", *ptrArray);

	printf("\n ptrArray[1]의 주소 (&ptrArray[1]) = %u", &ptrArray[1]);
	printf("\n ptrArray[1]의 값 (ptrArray[1]) = %u", ptrArray[1]);
	printf("\n ptrArray[1]의 가리키는 (*ptrArray[1]) = %c", *ptrArray[1]);
	printf("\n ptrArray[1]의 가리키는문자열(*ptrArray[1])= %s \n", *(ptrArray + 1));

	printf("\n ptrptr의 주소 (&ptrptr) = %u", &ptrptr);
	printf("\n ptrptr의 값 (ptrptr) = %u", ptrptr);
	printf("\n ptrptr의 1차 가리키는 ( *ptrptr) = %u", *ptrptr);
	printf("\n ptrptr의 2차 가리키는 (**ptrptr) = %c", **ptrptr);
	printf("\n ptrptr의 2차 가리키는문자열 (**ptrptr) = %s", *ptrptr);
